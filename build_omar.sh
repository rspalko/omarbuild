...
if [ $RUNOPTION -eq "1" -o $RUNOPTION -eq "3" ]; then
  ...
  packages=("geos" "proj4" "tiff" "geotiff" "openscenegraph" "postgresql" "postgis" "gdal" "ffmpeg" "cmake" "groovy" "grails" "tomcat" "jai" "openjpeg" "mrsid" "laszip" "kakadu" "mvapich2" "pdal" "ogg" "theora" "vorbis" "yasm" "x264" "x265" "vpx")
  urls=("${GEOS_URL}" "${PROJ4_URL}" "${TIFF_URL}" "${GEOTIFF_URL}" "${OPENSCENEGRAPH_URL}" "${POSTGRESQL_URL}" "${POSTGIS_URL}" "${GDAL_URL}" "${FFMPEG_URL}" "${CMAKE_URL}" "${GROOVY_URL}" "${GRAILS_URL}" "${TOMCAT_URL}" "${JAI_URL}" "${OPENJPEG_URL}" "${MRSID_URL}" "${LASZIP_URL}" "${KAKADU_URL}" "${MVAPICH2_URL}" "${PDAL_URL}" "${OGG_URL}" "${THEORA_URL}" "${VORBIS_URL}" "${YASM_URL}" "${X264_URL}" "${X265_URL}" "${VPX_URL}")
